...
export default function StaffWorkspace() {
  ...
  const onSubmit = async (data: StaffFormValues) => {
  // Guard: Prevent submissions if context UUIDs haven't resolved yet
  if (!normalizedOrgId || !normalizedBusinessId) {
    console.error("Submission blocked: Organization or Business ID is missing.");
    return;
  }

  try {
    // Construct the pristine 6-field object expected by the backend
    const exactPayload = {
      ...
      password: data.password,
      role: data.role, // Ensure this exact string matches a valid backend Enum value
    };

    console.log("Sending payload to backend:", exactPayload);
    
    await registerStaff(exactPayload as any);
    reset(); 
  } catch (err) {
    console.error("Mutation submission failure:", err);
  }
