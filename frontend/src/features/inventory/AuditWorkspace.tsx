...
// import { AuditTableRow } from "@/features/inventory/AuditTableRow";
// import { useProducts } from "@/features/business/hooks/useProducts";
// import { Loader2, AlertCircle, Package, Plus, Search, CheckCircle2 } from "lucide-react";
// import { ProductResponse } from "@/lib/api/generated/models";
// import { useBusinessContext } from "@/features/business/hooks/useBusiness";
// import Link from "next/link";
...
//   // Extract, structure, and filter physical trackable stock metrics via memory cache
...
//     return physicalItemsOnly.filter(
//       (product) =>
...
//   return (
//     /* PARENT CANVAS ENGINE LAYER: Absolute page constraint setup. Freezes viewport scroll fields entirely */
//     <div className="w-full h-screen max-h-screen p-4 md:p-6 flex flex-col overflow-hidden bg-background select-none font-sans antialiased">
      
//       {/* 1. Header Information Control Center Block (~10% Height Context) */}
...
//           <h2 className="text-sm font-black uppercase tracking-wider text-foreground">
//             Stock Take & Adjustments Workspace
//           </h2>
//           <p className="text-muted text-[11px] leading-relaxed max-w-2xl">
//             Verify actual inventory matching physical stock shelves with system databases. 
//             Modifications reflect instantly. Discrepancies require structural verification codes.
//           </p>
//         </div>

//         {/* Tactical restocking call-to-action component */}
//         <div className="shrink-0 flex items-center">
//           <Link href={`/org/${organizationId}/${businessId}/stock/restock`} passHref legacyBehavior>
...
//       {/* 2. Structured Functional Table Architecture (Bound to exactly 90% operational height layout) */}
...
//         {/* INTERACTIVE COMPONENT HUB: Fixed Real-Time Data Pattern Stream Input */}
...
//         {/* STATIC COLUMN RIBBON LAYER: Always locked at the top of the inventory window stream */}
...
//           <div className="flex-1 min-w-[200px] md:min-w-[250px]">Product Configuration Meta</div>
...
//         {/* LIQUID DATA ROW STREAM VIEWPORT: The only structural layer authorized to execute scroll routines */}
...
//         {/* SYSTEM STATUS GRID SUMMARY FOOTER: Static positioning layer handling structural ledger summaries */}
//         <footer className="h-10 px-6 bg-surface/40 border-t border-border/60 flex items-center justify-between text-[10px] text-muted font-bold uppercase tracking-wider shrink-0 z-10 select-none">
//           <div className="flex items-center gap-2">
//             <span className="inline-block w-1.5 h-1.5 rounded-full bg-brand-accent animate-pulse" />
//             <span className="font-sans text-muted/90">Terminal Feed Synced</span>
//           </div>
//           <div className="font-mono text-muted/70 tracking-normal normal-case">
//             NetHub Stock Core v4.1
//           </div>
//         </footer>
...
export const AuditWorkspace: React.FC<AuditWorkspaceProps> = ({ businessId }) => {
  ...
  return (
    <main id="main-content" className="w-full h-screen max-h-screen p-4 md:p-6 flex flex-col overflow-hidden bg-background select-none font-sans antialiased">
      <script
      ...
      <header className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 shrink-0 border-b border-border/40">
        <div className="space-y-1 max-w-4xl">
          <h2 className="text-sm font-black uppercase tracking-wider text-foreground flex items-center gap-2">
            ...
            <span>Stock Take & Adjustments Workspace</span>
          </h2>
          <p className="text-muted text-[11px] leading-relaxed max-w-2xl">
            Verify actual inventory matching physical stock shelves with system databases. 
            Modifications reflect instantly. Discrepancies require structural verification codes.
      ...
      <div className="h-[90%] max-h-[90%] mt-4 bg-card border border-border/60 rounded-[1.5rem] shadow-lift flex flex-col overflow-hidden min-h-0">
        ...
        {/* STATIC METRIC TRACKER FOOTER: Anchored persistently to the baseline of the 90% boundary layout */}
        <footer className="h-10 px-6 bg-surface/40 border-t border-border/60 flex items-center justify-between text-[10px] text-muted font-bold uppercase tracking-wider shrink-0 z-10 select-none">
          <div className="flex items-center gap-2">
            <span className="inline-block w-1.5 h-1.5 rounded-full bg-brand-accent animate-pulse" />
            <span className="font-sans text-muted/90">Terminal Feed Synced</span>
