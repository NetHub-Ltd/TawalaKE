...
export const AuditTableRow: React.FC<AuditTableRowProps> = ({
  ...
  const getRowStyles = () => {
    if (status === "success" && !hasVariance) return "bg-brand-accent/5 border-brand-accent/20";
    if (hasVariance) return "bg-brand-secondary/5 border-brand-secondary/20";
    return "border-border/40 hover:bg-surface/30";
  };
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center px-4 py-4 gap-4">
        ...
        <div className="flex-1 md:min-w-[250px]">
          <h4 className="font-bold text-foreground">{product.label}</h4>
          <p className="text-muted font-bold uppercase tracking-wider mt-1">
            SKU: {product.attributes?.sku || "NO_SKU"} &bull;{" "}
            <span className="opacity-80">{product.category || "General"}</span> &bull;{" "}
            {product.attributes?.unit_of_measure || "Units"}
          </p>
        ...
        <div className="flex flex-col justify-center md:w-32">
          <label className="font-bold text-muted uppercase tracking-widest md:hidden mb-1">Book Stock</label>
          <span className="font-mono font-black text-muted bg-surface border border-border/40 px-3 py-1.5 rounded w-fit">
            {bookStock.toFixed(2)}
          </span>
        </div>
        ...
        <div className="flex flex-col justify-center md:w-36">
          <label className="font-bold text-muted uppercase tracking-widest md:hidden mb-1">Physical Count</label>
          <input
            type="number"
            step="any"
            onFocus={(e) => e.target.select()}
            ...
            {...register("quantity", {
              ...
              min: {
                value: 0,
                message: "Stock cannot fall below zero boundaries"
              }
            })}
            className="w-full font-mono font-bold px-3 py-1.5 bg-card border border-border/60 rounded text-foreground focus:outline-none focus:border-brand-primary/40 disabled:bg-surface disabled:text-muted transition-all shadow-sm"
          />
          {errors.quantity && (
            <p className="text-brand-secondary font-bold mt-1">
              {errors.quantity.message}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center md:w-36">
          {variance === 0 ? (
            <span className="inline-flex items-center gap-1.5 font-black text-brand-accent bg-brand-accent/10 border border-brand-accent/20 px-3 py-1 rounded-full uppercase tracking-wider">
              <Check className="w-3.5 h-3.5 stroke-[3]" /> Match
            </span>
          ) : (
            <span className={`inline-flex items-center gap-1.5 font-black px-3 py-1 rounded-full border uppercase tracking-wider ${
              variance > 0 
                ? "text-brand-secondary bg-brand-secondary/10 border-brand-secondary/20" 
                : "text-brand-primary bg-brand-primary/10 border-brand-primary/20"
            }`}>
        ...
        {/* Action Button Segment */}
        <div className="flex items-center justify-end md:w-28">
          {status === "saving" ? (
            <Loader2 className="w-5 h-5 text-brand-primary animate-spin" />
          ) : (
            <button
              type="submit"
              disabled={!isDirty && status !== "error"}
              className={`inline-flex items-center gap-2 px-4 py-2 font-bold uppercase tracking-wider rounded border transition-all ${
                isDirty 
                  ? "bg-foreground text-card border-transparent hover:scale-[1.02] active:scale-100 cursor-pointer shadow-sm" 
                  : "bg-surface text-muted border-border/40 cursor-not-allowed"
              }`}
            >
      ...
      {/* Accountability Shelf Drawer Input Layer */}
      {hasVariance && (
        <div className="px-4 pb-5 pt-3 border-t border-dashed border-border/40 bg-surface/10 flex flex-col md:flex-row gap-4 animate-in fade-in slide-in-from-top-1 duration-200">
          <div className="flex-1 md:max-w-xs">
            <label className="block font-bold text-muted mb-1.5 uppercase tracking-widest">Reason Code *</label>
            <select
              {...register("reason_code", {
                validate: (v) => !hasVariance || v !== "" || "Please select an audit justification code."
              })}
              className="w-full font-bold px-3 py-2 bg-card border border-border/60 rounded text-foreground focus:outline-none focus:border-brand-primary/40 cursor-pointer"
            >
              <option value="">-- Choose Audit Classification --</option>
              <option value="THEFT_SHOPLIFTING">Theft or Shoplifting</option>
              <option value="DAMAGED_IN_STORE">Damaged / Broken Shelf Inventory</option>
          ...
          <div className="flex-1">
            <label className="block font-bold text-muted mb-1.5 uppercase tracking-widest">Accountability Notes *</label>
            <input
              type="text"
              placeholder="Provide clear operational context explaining this variance discrepancy..."
              {...register("notes", {
              ...
              className="w-full px-3 py-2 bg-card border border-border/60 rounded text-foreground focus:outline-none focus:border-brand-primary/40 font-medium"
            />
            {errors.notes && (
              <p className="text-brand-primary font-bold mt-1">
      ...
      {/* System Warning Message Row Area */}
      {status === "error" && errorMessage && (
        <div className="bg-brand-primary/10 border-t border-brand-primary/20 px-4 py-2 font-bold text-brand-primary uppercase tracking-wide">
          {errorMessage}
        </div>
      )}
    </form>
