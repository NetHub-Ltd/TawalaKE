...
export function AssetComposer({
  ...
  const onFormSubmit = async (data: ProductForm) => {
    ...
    try {
      ...
      console.error("[AssetComposer] Submission failed:", error);
    }
  };
  ...
  return (
    <>
      ...
      >
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  {...register("label", {
                    required: "Required",
                    validate: (v) =>
                      (v?.trim().length ?? 0) > 0 || "Cannot be empty spaces",
                  })}
                  type="text"
        ...
        {/* ACTION FOOTER */}
        <div className="px-5 py-3.5 bg-surface/30 border-t border-border/40 flex items-center justify-end gap-3 select-none">
          <button
            type="button"
            onClick={onCancel}
