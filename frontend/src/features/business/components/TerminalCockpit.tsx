...
import {
  getStagedSaleId,
  clearStagedSaleId,
} from "@/features/sales/lib/stagedSale";
...
export default function TerminalCockpit({ businessId }: TerminalCockpitProps) {
  ...
  useEffect(() => {
    ...
    setStagedSaleIdState(getStagedSaleId(businessId));
  }, [businessId, cart.length]);
    ...
    <>
      ...
      <div className="absolute inset-0 flex flex-row overflow-hidden">
        ...
        <aside
          ...
          className="hidden lg:flex w-76 shrink-0 h-full overflow-hidden border-l border-border/40 bg-card/40 flex-col min-w-0"
        >
              ...
              onClick={async () => {
                try {
                  const res = await fetch(
                    `/api/v1/org/stores/sales/${stagedSaleId}/cancel-staged?businessId=${businessId}`,
                    { method: "POST" }
                  );
                  if (!res.ok) {
                    const b = await res.json().catch(() => ({}));
                    throw new Error(b.error || b.detail || "Cancel failed");
                  }
