...
/** Direct BFF → backend /stock/receive (no shared stockProxy). */
export async function POST(req: NextRequest) {
  ...
  const url = backendUrl("/stock/receive");
  try {
    ...
    console.error("[stock/receive upstream]", { url, status: upstream.status, data });
    ...
    const msg = e instanceof Error ? e.message : "Receive stock failed";
    console.error("[stock/receive network]", { url, msg });
