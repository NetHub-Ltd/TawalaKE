...
async def get_business(
    ...
    try:
        b = await service.get_business(business_id, user_id=user.id)
    except DomainError as exc:
        raise _map(exc) from exc
    return BusinessRead.model_validate(b)
...
async def update_business(
    ...
    try:
        b = await service.update_business(business_id, body, user_id=user.id)
    except DomainError as exc:
        raise _map(exc) from exc
    return BusinessRead.model_validate(b)
...
async def create_branch(
    ...
    try:
        b = await service.create_branch(business_id, body, user_id=user.id)
    except DomainError as exc:
        raise _map(exc) from exc
...
async def list_branches(
    ...
    try:
        rows = await service.list_branches(business_id, user_id=user.id)
    except DomainError as exc:
        raise _map(exc) from exc
...
async def create_location(
    ...
    try:
        loc = await service.create_location(branch_id, body, user_id=user.id)
    except DomainError as exc:
        raise _map(exc) from exc
...
async def list_locations(
    ...
    try:
        rows = await service.list_locations(branch_id, user_id=user.id)
    except DomainError as exc:
        raise _map(exc) from exc
